...
use crate::app::components::{labels, CardListModel, HasHeaderBarModel, HeaderImageShape, ImageShape, PageModel, PlaylistModel};
use crate::app::components::DetailsPageModel;
use crate::app::models::*;
use crate::app::state::SelectionContext;
use crate::app::state::{BrowserAction, BrowserEvent, PlaybackAction, PaginationTarget, SelectionState};
...
impl PageModel for ArtistDetailsModel {
    ...
    fn load_more(&self) {
        ...
        let Some(next_page) = state.browser.artist_state(&self.id).map(|s| s.next_page.clone()) else { return };
        drop(state);
    ...
    fn is_liked(&self) -> bool {
        self.app_model.get_state().browser.artist_state(&self.id)
            .map(|s| s.is_followed).unwrap_or(false)
    }
...
impl CardListModel for ArtistDetailsModel {
    fn get_store(&self) -> Option<impl Deref<Target = ListStore<CardModel>> + '_> {
        self.app_model.map_state_opt(|s| Some(&s.browser.artist_state(&self.id)?.albums))
    }

    fn load_more(&self) { PageModel::load_more(self); }
    fn refresh(&self) {}
    ...
    fn image_shape(&self) -> ImageShape { ImageShape::Square }
}
...
impl PlaylistModel for ArtistDetailsModel {
    ...
    fn play_song_at(&self, _pos: usize, id: &str) {
        ...
        let batch = SongBatch { songs: tracks, batch: Batch { offset: 0, batch_size: total, total } };
        self.dispatcher.dispatch(PlaybackAction::LoadPagedSongs(SongsSource::Artist(self.id.clone()), batch).into());
...
impl SimpleHeaderBarModel for ArtistDetailsModel {
    fn title(&self) -> Option<String> { PageModel::get_title(self) }
    fn title_updated(&self, event: &AppEvent) -> bool { PageModel::should_refresh_details(self, event) }
    fn selection_context(&self) -> Option<SelectionContext> { None }
    fn select_all(&self) {}
}
